...
export function InvestmentIntelligenceSections({ ticker, horizonYears, expectedMoic, realizedVol, evidenceGrade }: Props) {
  ...
  return (
    <div className="investment-intelligence">
      <div className="dd-section decision-live-header"><h3>Decision Header</h3>
        <p><strong>VALIDATION {validation?.validation_status ?? "STALE"}</strong> ・ DATA COVERAGE {coverageGrade} ({Math.round(coverageRatio * 100)}%) ・ MODEL FAMILY {reverse?.model_family ?? "unclassified"} ・ LIVE INTELLIGENCE age {liveAge == null ? "unknown" : `${liveAge} days`}</p>
      </div>
      <div className="dd-section">
        <h3>市場が織り込む成長</h3>
        <p className="detail-cagr"><span className="th-badge">Not used in ranking</span> Coreと同じ成長フェード・終端仮定で現在価格から逆算。時点 {reverse?.as_of ?? "—"}</p>
      ...
      {sections.map(([key, title]) => <div className="dd-section" key={key}><h3>{title}</h3>
        <p className="detail-cagr"><span className="th-badge">Not used in ranking</span> {data[key]?.source ?? "source未取得"} ・ as of {data[key]?.as_of ?? "—"}</p>
      ...
      <div className="dd-section"><h3>リスク縮小後のポジション上限（preview）</h3>
        <p className="detail-cagr"><span className="th-badge">Default OFF</span> Core確率をケリー式へ入れず、既存hard capを縮小する方向だけ。</p>
        {risk ? <DataBlock response={risk} /> : <p className="intelligence-empty">未取得</p>}
      </div>

      <div className="dd-section"><h3>JPY・税引後シナリオ</h3>
        <p className="detail-cagr"><span className="th-badge">User layer</span> 企業評価と分離。為替を予測せず、現在USDJPYの±10%を表示。</p>
